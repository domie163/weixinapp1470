    ...
    <div class="addEdit-block">
        <el-form
            ...
            <el-row>
                ...
                <el-col :span="12" v-if="sessionTable !='checi' ">
                    <el-form-item class="upload" v-if="type!='info' && !ro.checiPhoto" label="火车照片" prop="checiPhoto">
                        <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in (checiForm.checiPhoto || '').split(',')" :src="$base.url+item" width="100" height="100">
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.checiPhoto" label="火车照片" prop="checiPhoto">
                            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in (ruleForm.checiPhoto || '').split(',')" :src="$base.url+item" width="100" height="100">
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12"  v-if="sessionTable !='checi' ">
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="checiCollectionTypes">
                        <el-select v-model="ruleForm.checiCollectionTypes" :disabled="ro.checiCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in checiCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="checiCollectionValue">
                        <el-input v-model="ruleForm.checiCollectionValue"
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=checi_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.checiCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`checiCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.checiCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
