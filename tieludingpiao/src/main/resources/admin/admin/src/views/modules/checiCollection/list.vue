        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '火车类型' : ''">
                        <el-select v-model="searchForm.checiTypes" placeholder="请选择火车类型">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('checiCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('checiCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('checiCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('checiCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/tieludingpiao/upload/checiCollectionMuBan.xls"
                        >批量导入车次收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('checiCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="checiCollectionUploadSuccess"
                                :on-error="checiCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('checiCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入车次收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('checiCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "checiCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('checiCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="checiPhoto"
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.checiPhoto">
                                <img :src="$base.url+scope.row.checiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="checiCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.checiCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('checiCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('checiCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('checiCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'checi_collection',//当前表表名,
                        sumColum : 'checi_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'checi_collection_name',//分组字符串字段
                        // types : 'checi_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['checiCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "checiCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            checiCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "checiCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            checiCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "checiCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            checiCollectionTypes:checiCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
