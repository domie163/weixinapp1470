	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('checiOrder', this.checiOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.checiOrderPhoto ? this.detail.checiOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('checiOrderYuyue', {
									yonghuId: _this.user.id,
									checiOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/checiOrderYuyue/list');
							}
						}
					});
				},
