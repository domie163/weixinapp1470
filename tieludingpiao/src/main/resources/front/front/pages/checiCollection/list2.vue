	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.checiName" type="text" placeholder="车次"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('checiCollection','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
    ...
    <view @tap="onDetailTap(product)" class="listm flex flex-between"
        ...
        <view class="listmr" style="padding: 10rpx ;">
            ...
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('checiCollection','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('checiCollection','删除')" class="cuIcon-delete"
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 类型 字段 字典表查询方法
		  	let checiCollectionTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'checi_collection_types'
		  	});
		  	this.checiCollectionTypesList = checiCollectionTypes.data.list;
			checiCollectionTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = checiCollectionTypes.data.list
		...
		methods: {
			...
			onDetailTap(item) {
				this.$utils.jump(`../checi/detail?id=${item.checiId}`)
			},
			// 修改
			onUpdateTap(id) {
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`checiCollection`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('checiCollection', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.checiCollectionShenheTime=shijian;
                            await _this.$api.update(`checiCollection`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`checiCollection`, searchForm);
				// 如果是第一页数据置空
