	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.checiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('checiCollection', this.checiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.checiCollectionPhoto ? this.detail.checiCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('checiCollectionYuyue', {
									yonghuId: _this.user.id,
									checiCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/checiCollectionYuyue/list');
							}
						}
					});
				},
