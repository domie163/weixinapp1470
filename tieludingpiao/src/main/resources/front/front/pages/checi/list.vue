	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.checiName" type="text" placeholder="车次标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('checi','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('checi','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='checiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in checiTypesList" :key="index" :class="checiTypes===item.codeIndex?'tabActive':''"
                                  @tap="checiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="list"
    ...
    <view @tap="onDetailTap(product)" class="listm flex flex-between"
        ...
        <image
                ...
                :src="product.checiPhoto?baseUrl+product.checiPhoto.split(',')[0]:''"></image>
        <view class="listmr" style="padding: 10rpx ;">
            ...
            <view class="hide1" v-if="product.checiNewMoney"
                ...
                ￥{{product.checiNewMoney}}
            </view>
            <view class="hide1" v-else-if="product.checiValue"
                  :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"30rpx","lineHeight":"56rpx","borderStyle":"solid"}'>
                {{product.checiValue}}
            </view>
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('checi','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('checi','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 火车类型 字段 字典表查询方法
            async checiTypesSelect(){
                let checiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'checi_types',
                });
                checiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.checiTypesList = checiTypes.data.list
            },
            //当前表的 火车类型字段类别搜索
			checiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.checiTypes != 0) {
                    params.checiTypes = this.checiTypes
                }
                let _this = this
                let res = await this.$api.list(`checi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('checi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.checiShenheTime=shijian;
                            await _this.$api.update(`checi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.checiName) {
                    searchForm['checiName'] = this.searchForm.checiName
                }

                let res = await this.$api.list(`checi`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.checiContent!=null){
                        item.checiContent = item.checiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
