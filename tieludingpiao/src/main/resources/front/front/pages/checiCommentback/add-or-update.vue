        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.checiCommentbackText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`checiCommentback`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.checiId){
                this.ruleForm.checiId = options.checiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.checiCommentbackText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`checiCommentback`, this.ruleForm);
                } else {
                    await this.$api.save(`checiCommentback`, this.ruleForm);
                }
