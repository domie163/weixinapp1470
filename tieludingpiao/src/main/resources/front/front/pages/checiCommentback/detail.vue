	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.checiCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('checiCommentback', this.checiCommentbackId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.checiCommentbackPhoto ? this.detail.checiCommentbackPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('checiCommentbackYuyue', {
									yonghuId: _this.user.id,
									checiCommentbackId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/checiCommentbackYuyue/list');
							}
						}
					});
				},
