	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.checiCommentbackText" type="text" placeholder="评价内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('checiCommentback','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('checiCommentback','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <view style="font-size: 30rpx;display: flex;justify-content: space-between;">
                    ...
                    <view style="float: right;padding: 10rpx 10rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></view>
                        ...
                        <view class="address-box" style="padding: 10rpx;">
                            <text style="margin-left: 30rpx;">我的评价：{{item.checiCommentbackText}}</text>
                        </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`checiCommentback`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('checiCommentback', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.checiCommentbackShenheTime=shijian;
                            await _this.$api.update(`checiCommentback`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.checiCommentbackText) {
                    searchForm['checiCommentbackText'] = this.searchForm.checiCommentbackText
                }

                let res = await this.$api.page(`checiCommentback`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
